...
/**
 * Notificación toast que aparece arriba a la derecha y se auto-cierra.
 *
 * @param {'success'|'error'|'info'|'warning'} type
 * @param {string}   message
 * @param {Function} onClose  - Callback llamado al cerrarse
 * @param {number}   duration - Milisegundos antes de auto-cierre (default: 3000)
 */
const Toast = ({ type = 'info', message, onClose, duration = 3000 }) => {
  ...
  const config = {
    ...
    info: {
      ...
      colors: 'bg-[var(--accent-bg)] text-[var(--text-h)] border-[var(--accent-border)]',
      bar: 'bg-[var(--accent)]',
    },
  };
  ...
  const toast = (
    <div
      ...
      style={{
        /* Animación de entrada sin depender de Tailwind arbitrary */
    ...
    >
      ...
      {/* Keyframes inyectados inline una sola vez */}
      <style>{`
        @keyframes toastIn {
